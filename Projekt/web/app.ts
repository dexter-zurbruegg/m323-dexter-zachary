...
let pendingGameType: 'blackjack' | 'poker' = 'blackjack';

const openBettingModal = (gameType: 'blackjack' | 'poker'): void => {
  pendingGameType = gameType;
  const balance  = getBalance();
  const betInput = getElement<HTMLInputElement>('bet-input');
  betInput.max   = String(balance);
  betInput.value = String(Math.min(50, balance));
  getElement('bet-max-label').textContent = `Balance: $${balance}  ·  Max bet: $${balance}`;
  getElement('bet-value').textContent     = `$${betInput.value}`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  getElement('bet-confirm').addEventListener('click', () => {
    ...
    if (pendingGameType === 'blackjack') { switchToScreen('blackjack'); startBlackjackGame(bet); }
    else                                 { switchToScreen('poker');     startPokerGame(bet); }
  });
  getElement('bet-cancel').addEventListener('click', () => getElement('bet-modal').classList.remove('open'));
